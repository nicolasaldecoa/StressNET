...
This is not a contributor or maintainer checklist. Maintainer release notes live
in `README_MAINTAINERS.md`.
...
For the repository examples, assume the user has cloned the repository and is
running code from the repository root so paths like `examples/example_data/...`
resolve correctly.
...
```python
from examples.download_radial_1d_2d_inverted_9v import ensure_dataset_downloaded
